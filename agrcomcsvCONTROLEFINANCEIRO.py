...
def registrar_movimento():
    ...
    print("\nCategorias:")
    for i, categoria in enumerate(tiposcategoria, start=1):#printa as categorias e numera o index delas 
        print(f"{i} - {categoria}")
    while True:
        try:
...
def listar_movimentos():
    if not registro:
        ...
        return
    print("\n==== MOVIMENTOS ====")
    for i, movimento in enumerate(registro, start=1):
        sinal = "-" if float(movimento["valor"]) < 0 else "+"#escolhe qual sinal vai mostrar com base no valor
